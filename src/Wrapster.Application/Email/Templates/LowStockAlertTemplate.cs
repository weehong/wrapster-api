...
public sealed class LowStockAlertTemplate : IEmailTemplate
{
    ...
    public LowStockAlertTemplate(string productName, string barcode, int currentStock, int threshold)
    {
    ...
    public string Subject => $"Low Stock Alert: {_productName} ({_currentStock} remaining)";

    public string RenderHtml()
    {
        StringBuilder html = new();
        html.Append("<div style=\"font-family: sans-serif; max-width: 600px; margin: 0 auto;\">");
        html.Append("<h2 style=\"color: #d32f2f;\">Low Stock Alert</h2>");
        html.Append(
            $"<p>The following product has fallen below the stock threshold of <strong>{_threshold}</strong>:</p>");
        ...
        html.Append(
            $"<td style=\"padding: 8px; border: 1px solid #e0e0e0; color: #d32f2f; font-weight: bold;\">{_currentStock}</td></tr>");
        html.Append("</table>");
        html.Append("<p>Please restock this item as soon as possible.</p>");
