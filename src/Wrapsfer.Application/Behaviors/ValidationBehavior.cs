    ...
    where TResponse : Result, IValidationResult<TResponse>
{
    ...
    {
        IReadOnlyList<IValidator<TRequest>> validatorList =
            validators as IReadOnlyList<IValidator<TRequest>> ?? [.. validators];
        ...
        if (failures.Count != 0)
        {
            ...
            Error error = new(Error.Validation.Code, errorMessage, ErrorType.Validation);
